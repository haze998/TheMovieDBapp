...
class CastNetworkManager {
    ...
    func requestMovieActors(movieId: Int, mediaType: String, completion: @escaping (([Cast]?) -> Void)) {
        guard let apiURL = URL(string: "https://api.themoviedb.org/3/movie/505642/credits?api_key=aef19f83a7261debd6b9b8edfd7919ce&language=en-US") else { fatalError("Invalid URL") }
    ...
    func requestTVShowActors(tvShowId: Int, completion: @escaping (([Cast]?) -> Void)) {
        guard let apiURL = URL(string: "https://api.themoviedb.org/3/tv//100088/aggregate_credits?api_key=32ea20e318793cf10469df41ffe5990d&language=en-US") else { fatalError("Invalid URL") }
        let session = URLSession(configuration: .default)
        let task = session.dataTask(with: apiURL) { data, response, error in
            guard let data = data else { return }
            do {
